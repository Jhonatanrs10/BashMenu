...
# network
## Network can take arguments but its not required.
...
## Set font glyph ranges. Defaults to Latin-only. Dont forget to set font_file/font_file_text to font that supports these
## Probably dont enable all at once because of memory usage and hardware limits concerns
...
### Set the screen edge, this can be useful for special displays that dont update from top edge to bottom. This goes from 0 (left side) to 3 (top edge), counter-clockwise.
# fcat_screen_edge=0
...
### Dont swap origin if using GL_UPPER_LEFT. Helps with Ryujinx
# gl_dont_flip=1
